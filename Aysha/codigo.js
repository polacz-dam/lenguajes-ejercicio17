function CargarDatosEnTabla() {
    ...
    if (request.status === 200) {
        ...
        Array
        // Se obtiene el elemento HTML que representa el cuerpo de la tabla var tableBody = document.getElementById("productosBody");
        // Se vacía el contenido de la tabla
        var tableBody = document.getElementById("productosBody");
        tableBody.innerHTML = "";
        var filtro = document.getElementById("anyo")
    }
    // Se itera cada producto y se construye un nuevo elemento HTML para la fila de la tabla
    for (var i = 0; i < productos.length; i++) {
        var producto = productos[i];
        if (filtro.value == producto.getElementsByTagName("anio")[0].textContent || filtro.value == "") {
